...
export const runAction = async (config: Config): Promise<void> => {
  ...
  const actions: Record<Commands, () => Promise<string>> = {
    ...
    preview: () => previewAction({stack, config, onOutput}),
  };
...
interface PreviewActionArgs {
  config: Config;
  stack: Stack;
  onOutput: (msg: string) => void;
}

async function previewAction(args: PreviewActionArgs): Promise<string> {
  const { stack, config, onOutput } = args;
